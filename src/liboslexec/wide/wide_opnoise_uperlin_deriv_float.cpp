...
namespace {
template<> struct BatchedCGPolicy<Param::WDF, Param::WDF, Param::WDF> {
    static constexpr int simd_threshold = 4;
};
template<> struct BatchedCGPolicy<Param::WDF, Param::WDV> {
    static constexpr int simd_threshold = 5;
};
template<> struct BatchedCGPolicy<Param::WDF, Param::WDV, Param::WDF> {
...
#define __OSL_XMACRO_ARGS (noise, NoiseScalar, Noise)

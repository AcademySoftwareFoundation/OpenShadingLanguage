...
namespace sfm {
    ...
    struct Multiplier {
        static OSL_FORCEINLINE float multiply(float value) { return value*(static_cast<float>(MT)/static_cast<float>(DivisorT)) ; }
    ...
    struct ProxyElement {
        ...
        OSL_FORCEINLINE float to_float() const { return static_cast<float>(MT)/static_cast<float>(DivisorT); }
    };
    ...
    struct ProxyElement<0, DivisorT> {
        ...
        OSL_FORCEINLINE ProxyElement<OtherMt, OtherDivisorT> operator +
    ...
    struct ProxyElement<MT, 1> {
        ...
        OSL_FORCEINLINE float operator + (ProxyElement<OtherDivisorT, OtherDivisorT>) const
        {
            return static_cast<float>(MT + 1);
        }

        template<int OtherMT>
        OSL_FORCEINLINE ProxyElement<OtherMT+MT, 1> operator + (ProxyElement<OtherMT, 1>) const
        {
            return ProxyElement<OtherMT+MT, 1>();
        }

        template<int OtherDivisorT>
        OSL_FORCEINLINE ProxyElement<MT, 1> operator + (ProxyElement<0, OtherDivisorT>) const
        {
    ...
    struct ProxyElement<DivisorT, DivisorT> {
        ...
        template<int OtherDivisorT>
        OSL_FORCEINLINE ProxyElement<2, 1> operator + (ProxyElement<OtherDivisorT, OtherDivisorT>) const
        {
            return ProxyElement<2, 1>();
        }

        template<int OtherDivisorT>
        OSL_FORCEINLINE ProxyElement<0, DivisorT> operator + (ProxyElement<0, OtherDivisorT>) const
        {
            return ProxyElement<0, DivisorT>();
        }
    ...
    template<typename ProxyElementX_T, typename ProxyElementY_T, typename ProxyElementZ_T>
    OSL_FORCEINLINE Vec3
    unproxy_vec3(const ProxyVec3<ProxyElementX_T, ProxyElementY_T, ProxyElementZ_T> &a)
    {

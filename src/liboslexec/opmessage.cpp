    ...
    if (m) {
        if (m->name == name) {
            ...
            if (m->has_data())
                ...
                osl_errorfmt(sg,
                    OSL::ustringhash("message \"{}\" already exists (created here: {}:{}) cannot set again from {}:{}"),
                ...
                //     sourcefile.c_str(), sourceline);
                    osl_errorfmt(sg,
                    OSL::ustringhash("message \"{}\" was queried before being set (queried here: {}:{}) setting it now ({}:{}) would lead to inconsistent results"),
                    name, m->sourcefile, m->sourceline,
        ...
        if (m->name == name) {
            if (m->type != type) {
                ...
                //     sourcefile.c_str(), sourceline);
                    ...
                    ustringhash data_arg;
                    ustringhash color_arg;
                    ustringhash closure_arg;

                    if(m->has_data())
                    {
                            data_arg = ustringhash_from(USTR("created"));
                    }
                    else
                    {
                            data_arg = ustringhash_from(USTR("queried"));
                    }
                    if(m->type == TypeDesc::PTR) {
                       color_arg = ustringhash_from(USTR("closure color"));
                    }
                    else{
                      color_arg = ustringhash_from(USTR(m->type.c_str()));
                    }

                    if(is_closure)
                    {
                    closure_arg = ustringhash_from(USTR("closure color"));
                    }
                    else
                    {
                        closure_arg = ustringhash_from(USTR(type.c_str()));
                    }

                     
                    osl_errorfmt(sg,
                    OSL::ustringhash("type mismatch for message \"{name}\" ({data} as {type} here: {sourcefile}:{sourceline}) cannot fetch as {closurearg} from {}:{}"),
                    name, data_arg,
                    color_arg,
            ...
            if (m->layeridx > layeridx) {
                ...
                osl_errorfmt(sg,
                    OSL::ustringhash("message \"{}\" was set by layer #{} ({}:{}) but is being queried by layer #{} ({}:{}) - messages may only be transferred from nodes that appear earlier in the shading network"),
                    name, m->layeridx, m->sourcefile,
                    m->sourceline, layeridx, sourcefile, sourceline);
                return 0;
            }
            // Message found!

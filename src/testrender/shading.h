...
using AbstractBSDF = bsdl::StaticVirtual<
    ...
    MxOrenNayarDiffuse, MxTranslucent, SpiThinLayer, HenyeyGreenstein>;
...
struct MediumParams {
    // Initialize to vaccum by default
...
struct MediumStack {
    ...
    OSL_HOSTDEVICE void compute_current_params()
    {
        ...
        new_params.sigma_s
            = { std::min(new_params.sigma_s.x, new_params.sigma_t.x),
    ...
    OSL_HOSTDEVICE MediumStack(const MediumStack& c)            = delete;
    OSL_HOSTDEVICE MediumStack& operator=(const MediumStack& c) = delete;

    enum { MaxEntries = 8 };

    MediumParams pool[MaxEntries];          // stable storage
...
struct ShadingResult {
    ...
    CompositeBSDF bsdf = {};
    // Tracks medium properties for both volumetric and dielectric surface boundaries.
    // The priority is used to resolve a 'winner' against neighboring
    // dielectric boundaries in `process_closure`. For false intersections,
    // winner IOR is used and surface shading is skipped.

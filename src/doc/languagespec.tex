...
\date{{\large Date: 19 October, 2009 \\
...
later time, we may consider making it possible for integrators themselves
...
\hline
\closurecolor\ {\ce \Ci} & Incident radience --- a closure representing the color of the light leaving the surface from \P in the direction {\cf -I}. \\
\hline
...
\apiitem{\colorclosure\ {\ce cooktorrance} (normal N, float roughness)}
...
\apiitem{\colorclosure\ {\ce ward} (normal N, vector xdir, vector ydir, float xrough, float yrough)}
\indexapi{ward()}
...
\colorclosure\ {\ce reflection} (float angle) \\
...
\colorclosure\ {\ce refraction} (float eta, float angle) \\
...
\apiitem{\colorclosure\ {\ce emission} (\emph{...params...}) \\
\colorclosure\ {\ce emission} (vector axis, float angle, \emph{...params...})}
\indexapi{emission}
...
to adjust the desired power density of your light.

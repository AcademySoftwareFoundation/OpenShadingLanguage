...
#undef IS_PTR
    ...
    __forceinline__ __device__ float3 make_float3(const float4& a)
    {
    ...
    extern __device__ float4 osl_tex2DLookup(void *handle, float s, float t);
             ...
             void *ustring_errormessage)
    {
        ...
        *((float3*)result) = make_float3(fromTexture.x, fromTexture.y, fromTexture.z);
        return 1;
    }

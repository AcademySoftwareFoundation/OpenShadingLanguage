    ...
    output color out_color)
{
    ...
    float float_val  = noise(noisetype, float_in, float_in2);
    color color_val  = noise(noisetype, float_in, float_in2);

    // Exercise the op masked
    if ((numStripes != 0) && (int(P[0]*P[0]*P[1]*(2*numStripes))%2==0))
    {
        float_val  = noise(noisetype, __OSL_XMACRO_STRIPE_TRANSFORM(float_in), __OSL_XMACRO_STRIPE_TRANSFORM2(float_in2));
        color_val  = noise(noisetype, __OSL_XMACRO_STRIPE_TRANSFORM(float_in), __OSL_XMACRO_STRIPE_TRANSFORM2(float_in2));
    }
...
shader __OSL_CONCAT(test_noise2d_, __OSL_XMACRO_SUFFIX)(
    ...
    output color out_color = 1)
{
    if (v < 0.49) {
        ...
        if (u < 0.24)
            if (u < 0.125)
                do_noise2d("perlin", numStripes, derivs, out_float, out_color);
            else
                do_noise2d("snoise", numStripes, derivs, out_float, out_color);
        else if (u > 0.26 && u < 0.49)
            if (u < 0.375)
                do_noise2d("uperlin", numStripes, derivs, out_float, out_color);
            else
                do_noise2d("noise", numStripes, derivs, out_float, out_color);
        else if (u > 0.51 && u < 0.74)
            do_noise2d("simplex", numStripes, derivs, out_float, out_color);
        else if (u > 0.76)
            do_noise2d("usimplex", numStripes, derivs, out_float, out_color);
    } else if (v > 0.51) {
        // color noise in 1,2,3,4 dimensions
        if (u < 0.24)
            do_noise2d("cell", numStripes, derivs, out_float, out_color);
        else if (u > 0.26 && u < 0.49)
            do_noise2d("hash", numStripes, derivs, out_float, out_color);
        else if (u > 0.51 && u < 0.74)
            do_noise2d("gabor", numStripes, derivs, out_float, out_color);
    }
}

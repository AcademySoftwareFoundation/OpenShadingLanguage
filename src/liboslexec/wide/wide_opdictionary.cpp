...
OSL_BATCHOP int
__OSL_OP(dict_find_iis)
(   void *bsg_, int nodeID, void *query)
{
...
OSL_BATCHOP void
__OSL_MASKED_OP3(dict_find,Wi,Wi,Ws)
(   void *bsg_, void *wout, void *wnodeID, void *wquery, unsigned int mask_value)
{
...
OSL_BATCHOP int
__OSL_OP(dict_find_iss)
(   void *bsg_, void *dictionary, void *query)
{
    auto *bsg = reinterpret_cast<BatchedShaderGlobals *>(bsg_);
...
OSL_BATCHOP void
__OSL_MASKED_OP3(dict_find,Wi,Ws,Ws)
(   void *bsg_, void *wout, void *wdictionary, void *wquery, unsigned int mask_value)
{
...
OSL_BATCHOP void
__OSL_MASKED_OP(dict_next)
(   void *bsg_, void *wout, void *wNodeID, unsigned int mask_value)
{
    auto *bsg = reinterpret_cast<BatchedShaderGlobals *>(bsg_);
...
OSL_BATCHOP int
__OSL_OP(dict_value)
(   void *bsg_, int nodeID, void *attribname,
    long long type, void *data)
{
    auto *bsg = reinterpret_cast<BatchedShaderGlobals *>(bsg_);
    return bsg->uniform.context->dict_value (nodeID, USTR(attribname), TYPEDESC(type), data);
}

namespace { // anonymous

template<typename ValueT, bool IsArrayT>
struct DictValueGetter;

template<typename ValueT>
struct DictValueGetter<ValueT, false>
{
    static void get(ShadingContext* context,
        Wide<const int> wNID,
        Wide<const ustring> wAttribName,
        MaskedData wdest,
        Masked<int> wout)
    {
        ...
        wdest.mask().foreach([=](ActiveLane lane)->void {
            ...
            ValueT value;
            int result = context->dict_value (nodeID, attribname, wdest.type(), &value);
            wout[lane] = result;
            if (result) {
                dest[lane] = value;
            }
...
template<typename ValueT>
struct DictValueGetter<ValueT, true>
{
        ...
        Masked<int> wout)
    {
        ...
        wdest.mask().foreach([=](ActiveLane lane)->void {
            ...
            int result = context->dict_value (nodeID, attribname, wdest.type(), &value[0]);
...
OSL_BATCHOP void
__OSL_MASKED_OP(dict_value)
(   void *bsg_, void *wOut/*return int dict_value*/,
    void *wNodeID, void *wAttribname,
    long long type, void *wDataValue /*attribute value*/,

    ...
    output color out_color)
{
    ...
    float float_val  = noise(noisetype, point_in);
    color color_val  = noise(noisetype, point_in);

    // Exercise the op masked
    if ((numStripes != 0) && (int(P[0]*P[0]*P[1]*(2*numStripes))%2==0))
    {
        float_val  = noise(noisetype, __OSL_XMACRO_STRIPE_TRANSFORM(point_in));
        color_val  = noise(noisetype, __OSL_XMACRO_STRIPE_TRANSFORM(point_in));
    }
...
shader __OSL_CONCAT(test_noise3d_, __OSL_XMACRO_SUFFIX)(
    ...
    output color out_color = 1)
{
    if (v < 0.49) {
        ...
        if (u < 0.24)
            if (u < 0.125)
                do_noise3d("perlin", numStripes, derivs, out_float, out_color);
            else
                do_noise3d("snoise", numStripes, derivs, out_float, out_color);
        else if (u > 0.26 && u < 0.49)
            if (u < 0.375)
                do_noise3d("uperlin", numStripes, derivs, out_float, out_color);
            else
                do_noise3d("noise", numStripes, derivs, out_float, out_color);
        else if (u > 0.51 && u < 0.74)
            do_noise3d("simplex", numStripes, derivs, out_float, out_color);
        else if (u > 0.76)
            do_noise3d("usimplex", numStripes, derivs, out_float, out_color);
    } else if (v > 0.51) {
        // color noise in 1,2,3,4 dimensions
        if (u < 0.24)
            do_noise3d("cell", numStripes, derivs, out_float, out_color);
        else if (u > 0.26 && u < 0.49)
            do_noise3d("hash", numStripes, derivs, out_float, out_color);
        else if (u > 0.51 && u < 0.74)
            do_noise3d("gabor", numStripes, derivs, out_float, out_color);
    }
}

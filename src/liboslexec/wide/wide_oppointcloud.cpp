...
default_pointcloud_search(BatchedShaderGlobals* bsg,
    ustring filename, const void * wcenter_,
    ...
    PointCloudSearchResults & results)
{
    ...
    // TODO: evaluate if sg->context->alloc_scratch should be used instead?
     Partio::ParticleIndex *indices = (Partio::ParticleIndex *)OIIO_ALLOCA(size_t, max_points);
...
OSL_FORCEINLINE void
dispatch_pointcloud_search(BatchedShaderGlobals* bsg,
    ustring filename, const void * wcenter,
    Wide<const float> wradius, int max_points, bool sort,
    PointCloudSearchResults & results)
{
    auto * bsr = bsg->uniform.renderer->batched(WidthTag());
    if (bsr->is_overridden_pointcloud_search()) {
        return bsr->pointcloud_search(bsg, filename, wcenter,
                wradius, max_points, sort, results);
    } else {
        return default_pointcloud_search(bsg, filename, wcenter,
                wradius, max_points, sort, results);
    }
}

OSL_FORCEINLINE Mask
default_pointcloud_get(BatchedShaderGlobals* bsg,
    ustring filename, Wide<const int[]> windices, Wide<const int> wnum_points,
    ustring attr_name, MaskedData wout_data)
{
    ...
    if (attr != nullptr) {
        ...
        is_compatible_with_partio = compatiblePartioType(partio_type, element_type);
    ...
    Partio::ParticleIndex *indices = (Partio::ParticleIndex *)OIIO_ALLOCA(size_t, windices.length());
    ...
    wout_data.mask().foreach([=, &success](ActiveLane lane)->void {
        ...
        if (partio_type == OIIO::TypeString) {
            ...
            const_cast<Partio::ParticlesData*>(cloud)->data (*attr, count,
                    (const Partio::ParticleIndex *)indices, /*sorted*/ /*true*/false, (void*)strindices);
            ...
            const_cast<Partio::ParticlesData*>(cloud)->data (*attr, count,
                    (const Partio::ParticleIndex *)indices, /*sorted*/ /*true*/false, aos_buffer);
...
OSL_FORCEINLINE Mask
dispatch_pointcloud_get(BatchedShaderGlobals* bsg,
    ustring filename, Wide<const int[]> windices, Wide<const int> wnum_points,
    ustring attr_name, MaskedData wout_data)
{
    auto * bsr = bsg->uniform.renderer->batched(WidthTag());
...
Mask default_pointcloud_write(BatchedShaderGlobals* bsg,
        ustring filename, Wide<const OSL::Vec3> wpos, int nattribs,
...
OSL_FORCEINLINE Mask
dispatch_pointcloud_write(BatchedShaderGlobals* bsg,
    ustring filename, Wide<const OSL::Vec3> wpos, int nattribs,
    const ustring *attr_names, const TypeDesc *attr_types,
    const void **ptrs_to_wide_attr_value, Mask mask)
{
    ...
    if (bsr->is_overridden_pointcloud_write()) {
        return bsr->pointcloud_write(bsg, filename, wpos, nattribs,
                attr_names, attr_types, ptrs_to_wide_attr_value, mask);
    } else {
...
OSL_BATCHOP void __OSL_MASKED_OP(pointcloud_search) (
        BatchedShaderGlobals* bsg, void *wout_num_points_, const char *filename, const void *wcenter_, void * wradius_,
                               int max_points, int sort, void *wout_indices_, int indices_array_length,
                               void *wout_distances_, int distances_array_length, int distances_has_derivs,
                               int mask_value, int nattrs, ...)
{
    PointCloudSearchResults pcsr{wout_num_points_, wout_indices_, indices_array_length, wout_distances_, distances_array_length, static_cast<bool>(distances_has_derivs), mask_value};

    ShadingContext* ctx = bsg->uniform.context;
    if (ctx->shadingsys().no_pointcloud())  // Debug mode to skip pointcloud expense
    {
    ...
    dispatch_pointcloud_search(bsg,
            USTR(filename), wcenter_,
            wradius, max_points, sort,
            pcsr);
    ...
    if (nattrs > 0) {
        ...
        for (int i = 0; i < nattrs; i++) {
            ...
            dispatch_pointcloud_get(bsg, USTR(filename), windices, wnum_points, attr_name, MaskedData{attr_type, false, Mask{mask_value}, out_data});
        }
        va_end (args);
    }
...
OSL_BATCHOP int __OSL_MASKED_OP(pointcloud_get) (
        BatchedShaderGlobals* bsg, const char *filename, void *windices_,
        int indices_array_length, void *wnum_points_,
        const char *attr_name, long long attr_type_,void *wout_data_, int mask_value)
{
    ShadingContext* ctx = bsg->uniform.context;
    if (ctx->shadingsys().no_pointcloud()) // Debug mode to skip pointcloud expense
        return 0; // mask_value;
    ...
    Mask success = dispatch_pointcloud_get(bsg, USTR(filename), windices, wnum_points, USTR(attr_name), MaskedData{attr_type, false, Mask{mask_value}, wout_data_});
    return success.value();
}

OSL_BATCHOP int __OSL_MASKED_OP(pointcloud_write) (
        BatchedShaderGlobals* bsg, const char *filename, const void *wpos_,
                      int nattribs, const ustring *attr_names,
                      const TypeDesc *attr_types, const void **ptrs_to_wide_attr_value, int mask_value)
{
    ShadingContext* ctx = bsg->uniform.context;
    if (ctx->shadingsys().no_pointcloud()) // Debug mode to skip pointcloud expense
        return 0; // mask_value;
    ...
    Mask success = dispatch_pointcloud_write(bsg, USTR(filename), wpos, nattribs, attr_names, attr_types, ptrs_to_wide_attr_value, Mask(mask_value));
    return success.value();

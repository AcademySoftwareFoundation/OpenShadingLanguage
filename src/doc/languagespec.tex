...
\fancyfoot[C]{\bfseries Unnamed Shading Language Specification}
...
\date{{\large Date: 9 January, 2009}}
...
\def\colorclosure{{\cf color closure}\xspace}
...
A shader is a program, with inputs and outputs, that performs
a specific task.  The program code is written in \langname, the
...
also be given new values by the renderer at runtime.  Below is a
...
\item Sample: given just an input (or output) direction, choose a
  ...
  directions.
\item Evaluate: given input and input light directions, compute the
...
The renderer contains a number of \emph{integrator} (selectable via the
...
\begin{annotate}
At present, this spec is written as if the integrators are all built
into the renderer itself.  At a later time, we will fully spec it
out so that integrators themselves may be written in \langname and
therefore users may expand the repertoire of integration techniques
...
break closure color continue do else emit float for if int lights matrix
normal point output public return string struct trace vector void while
...
bool case char class const default double enum extern false friend
inline long private protected short signed sizeof static struct
switch template this true typedef uniform union unsigned varying virtual
...
the comma or closing parenthesis that terminates the parameter
description.
...
the \qkw{reflectionblur} paramter; zero reflectivity would gray-out
...
In general, the type ``\emph{gentype} \closure'' behaves exactly like a
...
\apiitem{\emph{type} \textbf{texture} (string filename, float s, float t, \emph{...params...}) \\
\emph{type} \textbf{texture} (string filename, float s, float t, \\
\spc\spc\spc float dsdx, float dtdx, float dsdy, float dtdy, \emph{...params...})}
\indexapi{texture()}
...
\apiitem{\colorclosure\ {\ce cooktorrance} (float roughness)}
...
\apiitem{\colorclosure\ {\ce ward} (vector xdir, vector ydir, float xrough, float yrough)}
\indexapi{ward()}

Returns a \colorclosure that represents the anisotropic specular
reflectance of the surface at \P.  The \emph{xdir} and \emph{ydir}
vectors, both presumed to be unit-length and perpendicular to \N, 
...
in the \emph{xdir} and \emph{ydir} directions, respectively.
...
\apiitem{\colorclosure\ {\ce reflection} () \\
\colorclosure\ {\ce reflection} (string source) \\
\colorclosure\ {\ce reflection} (string source, float angle)}
\indexapi{reflection()}

Returns a \colorclosure that represents sharp mirror-like reflection
from the surface.  The reflection direction will be automatically
computed to be

\begin{code}
      reflect (normalize(I), normalize(faceforward(N,I)))
\end{code}
\vspace{-12pt}
...
\apiitem{\colorclosure\ {\ce refraction} (float eta) \\
\colorclosure\ {\ce refraction} (string source, float eta) \\
\colorclosure\ {\ce refraction} (string source, float eta, float angle)}
\indexapi{refraction()}

Returns a \colorclosure that represents sharp glass-like refraction of
objects ``behind'' the surface.  The refraction direction will be
automatically computed to be

\begin{code}
      normal Nf = normalize(faceforward(N,I));
      return refract (normalize(I), Nf, dot(I,N) < 0 ? eta : 1/eta);
\end{code}
\vspace{-12pt}

\noindent where \emph{eta} is the ratio of the index of refraction of
the medium on the ``inside'' of the surface divided by the index of
refration of the medium on the ``outside'' of the surface.  The
``outside'' direction is the one in the direction of the geometric
primitive's normals.
...
\apiitem{\colorclosure\ {\ce refraction} (vector R) \\
\colorclosure\ {\ce refraction} (string source, vector R) \\
\colorclosure\ {\ce refraction} (string source, vector R, float angle)}
\indexapi{refraction()}
...
\apiitem{\colorclosure\ {\ce translucence} ()}
...
Searches for the renderer attribute with the given \emph{name} (first
searching per-object attributes on the current object, then if not found
searching scene-wide attributes).  If found and it matches the type of
...
\begin{grammar}
<shader-file> ::= \{ <global-declaration> \} <shader-declaration>
...
<shader-formal-param> ::= <outputspec> <typename> <identifier>
                               <initializer> <metadata-block-opt>
\alt <outputspec> <typename> <identifier> <arrayspec>
...
<function-declaration> ::= \spc \\ <typename> <identifier> 
      "(" <function-formal-params-opt> ")" "{" <statement-list> "}"

<function-formal-params> ::= <function-formal-param> \{ "," <function-formal-param> \}

<function-formal-param> ::= <outputspec> <typename> <identifier> <arrayspec-opt>
...
<field-declaration> ::= <typename> <identifier> ";"
\alt <typename> <identifier> <arrayspec> ";"
...
<variable-declaration> ::= <typename> <def-expressions> ";"
...
<variable-ref> ::= <identifier> <array-deref-opt> <component-deref-opt> <incdec-op-opt>
...
<type-constructor> ::= <typename> "(" <expression-list> ")"
...
<typecast-expression> ::= "(" <typename> ")" <expression>

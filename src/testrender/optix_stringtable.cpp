...
OptiXStringTable::OptiXStringTable(optix::Context ctx)
    : m_ptr(nullptr), m_size(1 << 16), m_offset(0), m_optix_ctx(ctx)
{
}
...
void
OptiXStringTable::init(optix::Context ctx OSL_MAYBE_UNUSED)
{
#    ifdef OSL_USE_OPTIX
...
#        define STRDECL(str, var_name)                           \
            addString(ustring(str), ustring(OSL_NAMESPACE_STRING \
...
uint64_t
OptiXStringTable::addString(ustring str OSL_MAYBE_UNUSED,
...
int
OptiXStringTable::getOffset(const std::string& str) const
{

    ...
    output color out_color)
{
    ...
    float fperiod2 = __OSL_XMACRO_PERIOD_TRANSFORM_X2(v);

    // Exercise the op unmasked
    float float_val  = pnoise(noisetype, float_in, float_in2, fperiod, fperiod2);
    color color_val  = pnoise(noisetype, float_in, float_in2, fperiod, fperiod2);

    // Exercise the op masked
    if ((numStripes != 0) && (int(P[0]*P[0]*P[1]*(2*numStripes))%2==0))
    {
        float_val  = pnoise(noisetype, __OSL_XMACRO_STRIPE_TRANSFORM(float_in), __OSL_XMACRO_STRIPE_TRANSFORM2(float_in2), fperiod, fperiod2);
        color_val  = pnoise(noisetype, __OSL_XMACRO_STRIPE_TRANSFORM(float_in), __OSL_XMACRO_STRIPE_TRANSFORM2(float_in2), fperiod, fperiod2);
    }
...
shader __OSL_CONCAT(test_pnoise2d_, __OSL_XMACRO_SUFFIX)(
    ...
    output color out_color = 1)
{
    if (v < 0.49) {
        ...
        if (u < 0.24)
            do_pnoise2d("perlin", numStripes, derivs, out_float, out_color);
        else if (u > 0.26 && u < 0.49)
            do_pnoise2d("snoise", numStripes, derivs, out_float, out_color);
        else if (u > 0.51 && u < 0.74)
            do_pnoise2d("uperlin", numStripes, derivs, out_float, out_color);
        else if (u > 0.76)
            do_pnoise2d("noise", numStripes, derivs, out_float, out_color);
    } else if (v > 0.51) {
        // color noise in 1,2,3,4 dimensions
        if (u < 0.24)
            do_pnoise2d("cell", numStripes, derivs, out_float, out_color);
        else if (u > 0.26 && u < 0.49)
            do_pnoise2d("hash", numStripes, derivs, out_float, out_color);
        else if (u > 0.51 && u < 0.74)
            do_pnoise2d("gabor", numStripes, derivs, out_float, out_color);
    }
}

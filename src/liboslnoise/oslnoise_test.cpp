...
#define MAKE_IMAGE(noisename)                                              \
    for (int outdim = 1; outdim <= 3; outdim += 2) {                       \
        for (int indim = 1; indim <= 4; ++indim) {                         \
            ...
            img.write(Strutil::fmt::format("osl_{}_{}_{}.tif", #noisename, \
                                           outdim, indim));                \
        }                                                                  \
    }
...
test_perlin()
{
    ...
    static float results_3d[2 * N + 1] = { 0,         -0.298763, 0.12275,
                                           0.0533643, 0,         -0.106914,
                                           -0.61375,  -0.504605, 0 };
    static float results_4d[2 * N + 1] = { 0,          0.261616,  0.1043,
                                           -0.0740929, 0,         -0.0687704,
                                           -0.1043,    0.0645373, 0 };
    static Vec3 vresults_1d[2 * N + 1] = { Vec3(0, 0, 0),
                                           Vec3(0.316772, -0.416016, -0.267334),
                                           Vec3(0.3125, -0.75, -0.625),
                                           Vec3(0.0604248, -0.489258, -0.487061),
                                           Vec3(0, 0, 0),
                                           Vec3(0.211304, 0.43103, 0.351196),
                                           Vec3(0.5625, 0.5625, 0.1875),
                                           Vec3(0.467651, 0.247925, -0.124878),
                                           Vec3(0, 0, 0) };
    static Vec3 vresults_2d[2 * N + 1] = { Vec3(0, 0, 0),
                                           Vec3(0.0874839, 0.414739, 0.0248834),
                                           Vec3(0, 0.4962, -0.4962),
                                           Vec3(0.219915, 0.414739, -0.546962),
                                           Vec3(0, 0, 0),
                                           Vec3(-0.32144, -0.290742, 0.526898),
                                           Vec3(0.3308, 0, 0.4962),
                                           Vec3(0.567027, 0.290742, 0.14888),
                                           Vec3(0, -0, 0) };
    static Vec3 vresults_3d[2 * N + 1] = { Vec3(0, 0, 0),
                                           Vec3(-0.298763, -0.18861, 0.0753458),
                                           Vec3(0.12275, -0.491, 0),
                                           Vec3(0.0533643, -0.476305, -0.303479),
                                           Vec3(0, 0, 0),
                                           Vec3(-0.106914, 0.331779, 0.196719),
                                           Vec3(-0.61375, 0.36825, 0),
                                           Vec3(-0.504605, 0.0252173, 0.29568),
                                           Vec3(0, 0, 0) };
    static Vec3 vresults_4d[2 * N + 1]
        = { Vec3(0, 0, 0),
            Vec3(0.261616, -0.0846003, 0.353395),
            Vec3(0.1043, -0.1043, -0.1043),
            Vec3(-0.0740929, -0.168687, -0.333449),
            Vec3(0, 0, 0),
            Vec3(-0.0687704, 0.0508762, 0.224071),
            Vec3(-0.1043, -0.15645, 0),
            Vec3(0.0645373, -0.14268, -0.229348),
            Vec3(0, 0, 0) };
    for (int i = 0; i <= 2 * N; ++i) {
        // Signed perlin noise
    ...
    bench("  snoise(f,f)",
          [&]() { DoNotOptimize(snoise<float, float>(fval, fval)); });
    bench("  snoise(v)", [&]() { DoNotOptimize(snoise<const Vec3&>(vval)); });
    bench("  snoise(v,f)",
          [&]() { DoNotOptimize(snoise<const Vec3&, float>(vval, fval)); });

    bench("  vsnoise(f)", [&]() { DoNotOptimize(vsnoise<float>(fval)); });
    bench("  vsnoise(f,f)",
          [&]() { DoNotOptimize(vsnoise<float, float>(fval, fval)); });
    bench("  vsnoise(v)", [&]() { DoNotOptimize(vsnoise<const Vec3&>(vval)); });
    bench("  vsnoise(v,f)",
          [&]() { DoNotOptimize(vsnoise<const Vec3&, float>(vval, fval)); });
}
...
{
    const int N                        = 1;
    static float results_1d[2 * N + 1] = { 0.582426, 0.292355 };
    static float results_2d[2 * N + 1] = { 0.860313, 0.241469 };
    static float results_3d[2 * N + 1] = { 0.611068, 0.149855 };
    static float results_4d[2 * N + 1] = { 0.111573, 0.646492 };
    static Vec3 vresults_1d[2 * N + 1] = {
        Vec3(0.860313, 0.842521, 0.974821), Vec3(0.295013, 0.241469, 0.0633514)
    };
    static Vec3 vresults_2d[2 * N + 1] = { Vec3(0.611068, 0.185824, 0.0413061),
                                           Vec3(0.73866, 0.149855, 0.984101) };
    static Vec3 vresults_3d[2 * N + 1] = { Vec3(0.111573, 0.3251, 0.339947),
                                           Vec3(0.576891, 0.646492, 0.270396) };
    static Vec3 vresults_4d[2 * N + 1] = { Vec3(0.14575, 0.0431595, 0.531032),
                                           Vec3(0.847126, 0.733529, 0.911791) };
    ...
    bench("  cellnoise(f,f)",
          [&]() { DoNotOptimize(cellnoise<float, float>(fval, fval)); });
    bench("  cellnoise(v)",
          [&]() { DoNotOptimize(cellnoise<const Vec3&>(vval)); });
    bench("  cellnoise(v,f)",
          [&]() { DoNotOptimize(cellnoise<const Vec3&, float>(vval, fval)); });

    bench("  vcellnoise(f)", [&]() { DoNotOptimize(vcellnoise<float>(fval)); });
    bench("  vcellnoise(f,f)",
          [&]() { DoNotOptimize(vcellnoise<float, float>(fval, fval)); });
    bench("  vcellnoise(v)",
          [&]() { DoNotOptimize(vcellnoise<const Vec3&>(vval)); });
    bench("  vcellnoise(v,f)",
          [&]() { DoNotOptimize(vcellnoise<const Vec3&, float>(vval, fval)); });
}
...
{
    const int N                        = 1;
    static float results_1d[2 * N + 1] = { 0.983315, 0.914303 };
    static float results_2d[2 * N + 1] = { 0.716016, 0.360764 };
    static float results_3d[2 * N + 1] = { 0.311887, 0.273953 };
    static float results_4d[2 * N + 1] = { 0.480826, 0.84257 };
    static Vec3 vresults_1d[2 * N + 1] = { Vec3(0.253513, 0.658608, 0.718555),
                                           Vec3(0.673784, 0.825515, 0.827693) };
    static Vec3 vresults_2d[2 * N + 1] = { Vec3(0.172726, 0.779528, 0.780237),
                                           Vec3(0.431263, 0.443849, 0.592155) };
    static Vec3 vresults_3d[2 * N + 1] = { Vec3(0.59078, 0.79799, 0.0202766),
                                           Vec3(0.593547, 0.156555, 0.978352) };
    static Vec3 vresults_4d[2 * N + 1] = { Vec3(0.297735, 0.240833, 0.386421),
                                           Vec3(0.642915, 0.367309, 0.879035) };

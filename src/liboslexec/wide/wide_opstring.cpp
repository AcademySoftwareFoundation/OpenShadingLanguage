                                     ...
                                     unsigned int mask_value)
{
    ...
    wR.mask().foreach (
        [=, &local_buf, &heap_buf, &heap_buf_len](ActiveLane lane) -> void {
            ...
            ustring t  = ustring_from(wT[lane]);
            size_t sl  = s.length();
            size_t tl  = t.length();
            size_t len = sl + tl;
            char* buf  = local_buf;
                                         ...
                                         unsigned int mask_value)
{
    ...
    wR.mask().foreach ([=](ActiveLane lane) -> void {
        ustring substr = ustring_from(wSubs[lane]);
        ustring s      = ustring_from(wS[lane]);
...
{
    Wide<const ustringhash> wS(ws_);
    Wide<const ustringhash> wSubs(wsubs_);
    Masked<int> wR(wr_, Mask(mask_value));
...
__OSL_MASKED_OP2(stoi, Wi, Ws)(void* wint_ptr, void* wstr_ptr,
                               unsigned int mask_value)
{
    Wide<const ustringhash> wstr(wstr_ptr);
    ...
    wR.mask().foreach ([=](ActiveLane lane) -> void {
        const char* str = ustring_from(unproxy(wstr[lane])).c_str();
...
__OSL_MASKED_OP2(stof, Wf, Ws)(void* wr_, void* ws_, unsigned int mask_value)
{
    ...
    wR.mask().foreach ([=](ActiveLane lane) -> void {
        const char* str = ustring_from(unproxy(wS[lane])).c_str();
        // TODO: Suspect we could implement SIMD friendly version
        // that is more efficient than the library call
...
OSL_BATCHOP int
__OSL_OP(regex_impl)(void* bsg_, ustringhash_pod subject_, void* results,
                     int nresults, ustringhash_pod pattern, int fullmatch)
{
    ...
    OSL_ASSERT(ustring::is_unique(ustring_from(subject_).c_str()));
    OSL_ASSERT(ustring::is_unique(ustring_from(pattern).c_str()));
    ...
    if (nresults > 0) {
        ...
        for (int r = 0; r < nresults; ++r) {
            if (r / 2 < (int)mresults.size()) {
                ...
                m[r] = ustring_from(pattern).length();
            }
        }
        return res;
    } else {
                            ...
                            unsigned int mask_value)
{
    ...
    Wide<const ustringhash> wsubject(wsubject_ptr);
    Wide<const ustringhash> wpattern(wpattern_ptr);

    mask.foreach ([=](ActiveLane lane) -> void {
        ustring usubject = ustring_from(wsubject[lane]);
        ustring pattern  = ustring_from(wpattern[lane]);
...
__OSL_OP(format_uniform)(ustringhash_pod format_str, ...)
{
    va_list args;
    va_start(args, format_str);
    std::string s = Strutil::vsprintf(ustring_from(format_str).c_str(), args);
                  ...
                  ustringhash_pod format_str, ...)
{
    if (bsg->uniform.context->allow_warnings()) {
        ...
        va_list args;
        va_start(args, format_str);
        std::string s = Strutil::vsprintf(ustring_from(format_str).c_str(),
                                          args);
        va_end(args);
...
__OSL_OP(fprintf)(BatchedShaderGlobals* bsg, unsigned int mask_value,
                  ustringhash_pod filename, ustringhash_pod format_str, ...)
{
...
{
    Wide<const ustringhash> wS(wstr);
    ...
    Wide<const ustringhash> wSep(wsep);

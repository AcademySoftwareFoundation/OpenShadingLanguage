...
\begin{annotate}
At present, this document is written as if the integrators are all built
...
\begin{annotate}
Is a general term such as ``translucence'' too presumptuous for a simple
backfacing lambert?  Should we instead call this {\cf backdiffuse},
{\cf backlambert}, or something more specific?  Since probably later we
will add one or more physically-accurate, translucence function.
\end{annotate}
...
implemented it with ncomps != 3, and (2) if you really looked hard at
...
avoids two texure calls, as well as avoiding cluttering the language

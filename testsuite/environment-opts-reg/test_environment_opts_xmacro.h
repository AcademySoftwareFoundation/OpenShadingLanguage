...
shader __OSL_CONCAT(test_environment_opts_, __OSL_XMACRO_SUFFIX) (
      ...
      )
{
...

    {
        ...
        color C = environment (filename, vP, "alpha", a, "wrap", wrap_val);
        out_alpha_derivs = color (a, Dx(a)/Dx(u), Dy(a)/Dy(v));
    }

    {
        float b = init(pow (u/2, 2.0), 0.5);
        out_blur = (color) environment (filename, vP, "blur", b);
    }

    {
        out_color = (color) environment (filename, vP);
...

    {
        ...
        color C = (color) environment (filename, vP, "errormessage", err);
        if (err == "") {
...

    {
        ...
        out_firstchannel = (color) environment (filename, vP, "firstchannel", firstchannel_val, "fill", fill_val);
    }
    
    {
        float alpha = 0;
        float missingalpha_val = init(float(int(u*8+v*8))/16.0,0.75);
        float x = environment (filename, vP, "missingalpha", missingalpha_val,
                "alpha", alpha);
    ...
    
    {
        ...
        out_missing_color = (color) environment (filename, vP, "missingcolor", missingcolor_val);
    }
    
    {
        out_simple = (color) environment (filename, vP);
    }
    
    {
        ...
        out_smallderivs = (color) environment (filename, vP, vector(xwidth, 0, 0), vector(0, ywidth, 0), "blur", blur_val);
    }
...

    {
        ...
        out_width = (color) environment (filename, vP, "width", width_val);
    }

    {
        float xwidth = u*u * 10;
        float ywidth = v*v * 5;
        out_widthderivs = (color) environment (filename, vP, Dx(vP)*xwidth, Dy(vP)*ywidth);
    }

    {
        ...
        out_wrap = (color) environment (filename, point(-0.1 + 2 * u, -0.2 + 2 * v, -0.3 + (u+v)),
                "wrap", wrap_val);
    }
}

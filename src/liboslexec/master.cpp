...
ShaderMaster::ShaderMaster(ShadingSystemImpl& shadingsys)
    : m_shadingsys(shadingsys), m_range_checking(shadingsys.range_checking())
{
}
...
ShaderMaster::resolve_syms()
{
    ...
    for (auto&& s : m_symbols) {
        ...
        if (s.typespec().is_closure()) {
            ...
            s.size(alen
                   * sizeof(ClosureColor*));  // heap stores ptrs to closures
        ...
        if (s.symtype() == SymTypeConst) {
            if (s.dataoffset() >= 0) {
                if (s.typespec().simpletype().basetype == TypeDesc::INT)
                    s.set_dataptr(SymArena::Absolute,
                                  &(m_iconsts[s.dataoffset()]));
                else if (s.typespec().simpletype().basetype == TypeDesc::FLOAT)
                    s.set_dataptr(SymArena::Absolute,
                                  &(m_fconsts[s.dataoffset()]));
                else if (s.typespec().simpletype().basetype == TypeDesc::STRING)
                    s.set_dataptr(SymArena::Absolute,
                                  &(m_sconsts[s.dataoffset()]));
            }
        }
        ++i;
    }
...
ShaderMaster::print()
{
    ...
    out << "Shader " << m_shadername << " type=" << shadertypename() << "\n";
    out << "  path = " << m_osofilename << "\n";
    out << "  symbols:\n";
    for (size_t i = 0; i < m_symbols.size(); ++i) {
        const Symbol& s(m_symbols[i]);
        out << "    " << i << ": " << s.typespec().string() << " " << s.name()
            << "\n";
    }
    out << "  int consts:\n    ";
